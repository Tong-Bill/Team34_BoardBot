...
# Experimental version of AI file with added calls for emoji display & trash-talking
# Used for progress demo
# To be integrated into the main AI file at a later date - Bill Tong
...
def playerTurn():
	rules.gameSetup()
	boardPosition = 0
	# Begin game with neutral emotion displayed & neutral dialogue - Bill
	subprocess.call(['./neutral.sh'], shell=True)	
	expressions.Run(0)
	...
	for i in range(1, 11):
		...
		if (rules.evaluateDice(result)):
				...
				if (boardObj.boardObject[boardPosition][0] == "Property"):
						print("Landed on " + boardObj.boardObject[boardPosition][2])
						print("This property costs $" + str(boardObj.boardObject[boardPosition][3]))
						# Invoke angry emoji on screen & angry dialogue from list - Bill
						subprocess.call(['./angry.sh'], shell=True)
						expressions.Run(1)
				else: 
						print("Landed on " + boardObj.boardObject[boardPosition][1])
						# Invoke happy emoji on screen & happy dialogue from list - Bill
						subprocess.call(['./happy.sh'], shell=True)
						expressions.Run(3)
				...
				if "Jail" in boardObj.parseSpace(boardPosition):
						...
						# Sad emoji & sad dialogue if landed on jail - Bill
						subprocess.call(['./sad.sh'], shell=True)
						expressions.Run(4)
				if result[0] == result[1]:
						print("I rolled doubles! Taking another turn\n")
						# Sassy emoji & sassy dialogue if die happens to be the same - Bill
						subprocess.call(['./sassy.sh'], shell=True)
						expressions.Run(5)
