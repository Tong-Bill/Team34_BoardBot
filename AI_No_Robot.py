#!/usr/bin/env python
import rules_no_robot as rules
import subprocess
  ...
  def giveMoney(self, amount):
    ...
    if self.isPlayer == False:
      ...
      if self.robotMoneySum >= 0:
        ...
        print("Out of money! Trying to sell houses and properties!")
        return -1
        ...
        if color in self.playerOwnedSets:
          for name in self.playerOwnedProperties[color]:
            if self.playerOwnedProperties[color][name][1] != 0:
              ...
              # - Bill Tong
              subprocess.call(['./confused.sh'], shell = True)
              trump.dialogue(3)
              return
            else:
          ...
          for name in self.playerOwnedProperties[color]:
            if self.playerOwnedProperties[color][name][0] == True:
              prop[color].append(name)
              # - Bill Tong
      subprocess.call(['./worried.sh'], shell = True)
      trump.dialogue(4)
      print(prop)
  ...
  def playerDecisions(self):
    ...
    while endTurn == False:
      ...
      if choice == 1:
        ...
        print(self.playerMoneySum)
        print(type(self.playerMoneySum))
        print("^ type of playerMoneySum\n\n")
  ...
  def robotDecision(self):
    ...
    if not self.checkBankruptcy():
      if self.square == 0:
        ...
        trump.happy() 
      if self.robotMoneySum <= 200 and self.house > 0:
        self.sellHouse()
      else:
        self.mortgage()
...
def playTurn():
    """try:
      while True:
        ...
        global server
        global webInfoPublisher
        server = roslibpy.Ros(host='134.197.95.215', port=9090) # Assign connection info
        webInfoPublisher = roslibpy.Topic(server, '/playerinfo', 'std_msgs/Int16')
